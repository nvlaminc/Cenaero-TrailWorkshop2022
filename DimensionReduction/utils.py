...
def gen_dwt(sig, width= np.arange(1,25)):
    dwt = []
    # mean per participant for each week
    s = sig[:, :7*24*76].reshape(len(sig), -1, 7, 24).mean(1)
...
def plot_cluster(dataset, cluster, cluster_id=0, ploperiod='day'):
    fig = plt.figure()
    population = (cluster == cluster_id).sum()
    
    if period == 'day':
        # Reshape per days
        x = dataset[cluster==cluster_id].reshape(population, -1, 24)
        ...
        plt.suptitle(f'Daily Energy Consumption Profile of the {cluster_id+1} Cluster')
    
    elif period == 'week':
        # Reshape per weeks remove last days to have a multiple
        #of a week (i.e. 7 days).
        x = dataset[cluster==cluster_id][:, :7*24*76].reshape(population, -1, 7, 24)
        for i in range(population):
            plt.plot(x[i].mean(0).flatten(), c='black', linewidth=0.25, alpha=0.75)
        plt.plot(x.mean(1).mean(0).flatten())
        plt.suptitle(f'Weekly Energy Consumption Profile of the {cluster_id+1} Cluster')
